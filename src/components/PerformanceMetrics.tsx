...
function AnimatedCircle({ label, delay }: { label: string; delay: number }) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center gap-4">
      ...
      <span className="font-semibold text-navy dark:text-gray-300 tracking-wide">{label}</span>
    </div>
  );
}
...
export default function PerformanceMetrics() {
  return (
    <section className="py-16 border-b border-gray-100 dark:border-gray-800 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-10">
          <ScrollReveal delay={0}>
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/40 text-emerald-700 dark:text-emerald-400 font-semibold text-xs uppercase tracking-widest mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
              Flawless Execution
            </span>
          </ScrollReveal>
          <ScrollReveal delay={0.1}>
            <h2 className="text-3xl md:text-5xl font-bold text-navy dark:text-white mb-6 tracking-tight">
